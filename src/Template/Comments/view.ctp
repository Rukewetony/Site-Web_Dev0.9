...
<div class="comments view large-10 medium-9 columns">
    ...
    <div class="row">
        <div class="large-5 columns strings">
            ...
            <p><?= $comment->has('ticket') ? $this->Html->link($comment->ticket->id, ['controller' => 'Tickets', 'action' => 'view', $comment->ticket->id]) : '' ?></p>
        </div>
        <div class="large-2 columns numbers end">
            <h6 class="subheader"><?= __('Id') ?></h6>
            <p><?= $this->Number->format($comment->id) ?></p>
            <h6 class="subheader"><?= __('Is Spam') ?></h6>
            <p><?= $this->Number->format($comment->is_spam) ?></p>
        </div>
    ...
    <div class="row texts">
        <div class="columns large-9">
            ...
            <?= $this->Text->autoParagraph(h($comment->status)) ?>
        </div>
    </div>
    <div class="row texts">
        <div class="columns large-9">
